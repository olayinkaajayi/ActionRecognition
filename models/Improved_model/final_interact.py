...
class InteractFinal(nn.Module):
    ...
    def __init__(self, in_dim, hidden_size, dropout_rate=0.1):
        super(InteractFinal, self).__init__()
    ...
    def forward(self, x1, x2,cache=None):
        ...
        q1 = self.query1(x1) #[b x time x filter_size]
        k1 = self.key1(x1) #[b x time x filter_size]
        v1 = self.value1(x1) #[b x time x filter_size]

        q2 = self.query2(x2) #[b x time x filter_size]
        k2 = self.key2(x2) #[b x time x filter_size]
        v2 = self.value2(x2) #[b x time x filter_size]

        k2 = k2.transpose(-1, -2)  #[b x filter_size x time]
        k1 = k1.transpose(-1, -2)  #[b x filter_size x time]
        ...
        x1 = torch.matmul(q1, k2)  #[b x time x time]
        x1 = torch.softmax(x1, dim=-1)
        x1 = self.att_dropout1(x1)
        x1 = x1.matmul(v1)  #[b x time x filter_size]

        x2 = torch.matmul(q2, k1)  #[b x time x time]
        x2 = torch.softmax(x2, dim=-1)
        x2 = self.att_dropout2(x2)
        x2 = x2.matmul(v2)  #[b x time x filter_size]

        # You can sum up x1 and x2 as an effect of their interaction
        x = torch.concat([x1,x2],dim=-1) #[b x time x 2*filter_size]

        x = self.output_layer(x) #[b x time x filter_size]
