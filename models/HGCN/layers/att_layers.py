...
class DenseAtt(nn.Module):
    ...
    def forward (self, x, adj):
        n = x.size(-2) #formerly .size(0) we are focusing on the nodes
        # b x n x 1 x d
        x_left = torch.unsqueeze(x, -2) #formerly .unsqueeze(x, 1) we are focusing on the nodes
        x_left = x_left.expand(-1, -1, n, -1) # b x n x n x d

        # b x 1 x n x d
        x_right = torch.unsqueeze(x, -3) #formerly .unsqueeze(x, 0) we are focusing on the nodes
        x_right = x_right.expand(-1, n, -1, -1) # b x n x n x d

        # b x n x n x 2d
        x_cat = torch.cat((x_left, x_right), dim=-1) #formerly dim=2
        # b x n x n
        att_adj = self.linear(x_cat).squeeze() #why no LeakyReLU after this? Authors probably consider it unnecessary.
        # Masked Attention
        zero_vec  = -9e15*torch.ones_like(att_adj)
        att_adj = torch.where(adj > 0, att_adj, zero_vec) # b x n x n #formerlyadj.to_dense()

        att_adj = F.softmax(att_adj, dim=1) # b x n x n
