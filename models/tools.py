...
def valid_crop_resize(data_numpy,valid_frame_num,p_interval,window, max_time=64, rrot=True):
    # input: C,T,V,M
    ...
    if window == -1:
        # We use this when we don't want to resize
        data = torch.tensor(data_numpy,dtype=torch.float) # shape: C,T,V,M
    ...
    if len(p_interval) == 1:
        ...
        cropped_length = np.minimum(np.maximum(int(np.floor(valid_size*p)),max_time), valid_size)# constraint cropped_length lower bound as 64
    ...
    if not rrot:
        data = data.contiguous().view(C, V, M, window).permute(0, 3, 1, 2).contiguous() # shape: C,T,V,M
    else:
        data = data.contiguous().view(C, V, M, window).permute(0, 3, 1, 2).contiguous().numpy() # shape: C,T,V,M
