...
class InfoGraph(nn.Module):
    ...
    def forward(self, data, A= None, get_embed=False, get_nodes= False, batch=None):
        if A is None:
        ...
        if not get_embed:
            ...
            # idx_f_dim = np.random.permutation(self.dataset_num_features)
        ...
        if get_nodes:
            return self.encoder(x, edge_index, get_global=False) #node embeddings

        g_enc, l_enc = self.encoder(x, edge_index)
        if get_embed:
            return g_enc #graph-level embedding
