...
class MultiHeadAttention(nn.Module): # we set head_size=25 so that each node is handled individually
    def __init__(self, hidden_size, dropout_rate, head_size=25):
        ...
        #     self.linear_q.append(nn.Linear(hidden_size, att_size, bias=False))
        #     self.linear_k.append(nn.Linear(hidden_size, att_size, bias=False))
        #     self.linear_v.append(nn.Linear(hidden_size, att_size, bias=False))
        #     initialize_weight(self.linear_q[i])
        #     initialize_weight(self.linear_k[i])
        #     initialize_weight(self.linear_v[i])
    ...
    def forward(self, q, k, v,cache=None):
        ...
        #     hold.append(self.linear_q[i](q[:,:,i,:]).unsqueeze(-2)) # [b, q_len, 1, d_k]
        # q = torch.cat(hold,dim=-2) # [b, q_len, h, d_k]
        if cache is not None and 'encdec_k' in cache:
            ...
            # hold2 = []
            # for i in range(self.head_size):
            #     hold1.append(self.linear_k[i](q[:,:,i,:]).unsqueeze(-2)) # [b, q_len, 1, d_k]
            #     hold2.append(self.linear_v[i](q[:,:,i,:]).unsqueeze(-2)) # [b, q_len, 1, d_k]
            # k = torch.cat(hold1,dim=-2) # [b, q_len, h, d_k]
            # v = torch.cat(hold2,dim=-2) # [b, q_len, h, d_k]
