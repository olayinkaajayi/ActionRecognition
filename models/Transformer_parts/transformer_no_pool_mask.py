...
class MultiHeadAttention(nn.Module): # we set head_size=25 so that each node is handled individually
    def __init__(self, hidden_size, dropout_rate, head_size=25, divide=True):
        ...
        self.register_buffer('mask', self.get_mask(hidden_size,att_size), persistent=False)


    def get_mask(self,n,stride):
        '''This is a mask matrix that shuts off some weights in q,k and v.'''
        mask = torch.zeros((n,n))

        for s in range(0,n,stride):
            mask[s:s+stride,s:s+stride] = 1
    ...
    def forward(self, q, k, v,cache=None):
        ...
        self.linear_k.weight.data = self.linear_k.weight * self.mask #This would allow each node compute k from only itself.
        self.linear_q.weight.data = self.linear_q.weight * self.mask #This would allow each node compute q from only itself.
        self.linear_v.weight.data = self.linear_v.weight * self.mask #This would allow each node compute v from only itself.
        self.output_layer.weight.data = self.output_layer.weight * self.mask #This would allow each node compute x from only itself.
