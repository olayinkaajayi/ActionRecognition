...
class Position_encode(torch.nn.Module):
    ...
    def contrast_adj_n_degDist(self, Z, nodes):
        ...
        total = 0
        deg_dist_total = 0
        pos_neigh = self.neighbours(deg_dist=False)
        deg_dist_pos_neigh = self.neighbours(deg_dist=True)
        ...
        for i in nodes:
            ...
            # Degree distribution
            deg_dist_sum_pos = 0
            for p in deg_dist_pos_neigh[i]:
                num = self.hamming_dist(Z[i], Z[p]) / self.temperature
                deg_dist_sum_pos += num
            ...
            for a in neg_neigh:
                ...
                # forgot to take the exponential of the hamming distance
                # fixed on 8th Feb, 2023.
                deg_dist_sum_neg += torch.exp(den)


            log_den = torch.log(sum_neg) # Node distribution
            deg_dist_log_den = torch.log(deg_dist_sum_neg) # Degree distribution
            ...
            # Degree distribution
            deg_dist_len_pos_neigh = len(deg_dist_pos_neigh[i])
            if (deg_dist_len_pos_neigh == 0):
                deg_dist_len_pos_neigh = 1.0 #To avoide dividing by zero. This will not contribute to the result

            deg_dist_total += (- deg_dist_sum_pos / deg_dist_len_pos_neigh) + deg_dist_log_den


        return total/self.N, deg_dist_total/self.N
    ...
    def forward(self, selected_nodes=None, test=False, deg=False):
        ...
        Z = torch.sigmoid(self.P)
        # Decide whether to put torch.round() here
